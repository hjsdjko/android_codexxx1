...
import com.design.appproject.ui.tuifangxinxi.ListFilterDialog
...
import com.design.appproject.databinding.TuifangxinxicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.tuifangxinxi.ListModel
import com.design.appproject.ui.tuifangxinxi.ListAdapter
 ...
 * 退房信息列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_TUIFANGXINXI)
class ListFragment : BaseBindingFragment<TuifangxinxicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("退房信息",mHasBack)
        setBarColor("#C6000F","white")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("tuifangxinxi","新增"))|| Utils.isAuthFront("tuifangxinxi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_TUIFANGXINXI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("tuifangxinxi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_TUIFANGXINXI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun TuifangxinxicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("tuifangxinxi", requestParams)
        }.otherwise {
                mListModel.list("tuifangxinxi", requestParams)
        }
    }
}