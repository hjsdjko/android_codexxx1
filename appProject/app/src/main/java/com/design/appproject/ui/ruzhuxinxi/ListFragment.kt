...
import com.design.appproject.ui.ruzhuxinxi.ListFilterDialog
...
import com.design.appproject.databinding.RuzhuxinxicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.ruzhuxinxi.ListModel
import com.design.appproject.ui.ruzhuxinxi.ListAdapter
 ...
 * 入住信息列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_RUZHUXINXI)
class ListFragment : BaseBindingFragment<RuzhuxinxicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("入住信息",mHasBack)
        setBarColor("#C6000F","white")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("ruzhuxinxi","新增"))|| Utils.isAuthFront("ruzhuxinxi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_RUZHUXINXI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("ruzhuxinxi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_RUZHUXINXI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun RuzhuxinxicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("ruzhuxinxi", requestParams)
        }.otherwise {
                mListModel.list("ruzhuxinxi", requestParams)
        }
    }
}