...
import com.design.appproject.databinding.KefangxinxiFilterDialogLayoutBinding
...
class ListFilterDialog(context: Context) : DrawerPopupView(context) {
    ...
    lateinit var binding: KefangxinxiFilterDialogLayoutBinding
    ...
    override fun onCreate() {
        ...
        binding.apply {
            ...
            "已预约,未预约".split(",").forEachIndexed {index, s ->
                fangjianzhuangtaiFbl.addView(createCustomItemView(index,s,fangjianzhuangtaiFbl))
            }
            resetBtn.setOnClickListener { //重置
                jiageStartEt.setText("")
                jiageEndEt.setText("")
                fangjianzhuangtaiFbl.children.forEach { it.isSelected=false }
                callBackListener?.invoke(false,null)
            }
            ensureBtn.setOnClickListener { //确定
                params.clear()
                jiageStartEt.text.toString().isNotNullOrEmpty().yes {
                    params.put("jiagestart",jiageStartEt.text.toString())
                }
                jiageEndEt.text.toString().isNotNullOrEmpty().yes {
                    params.put("jiageend",jiageEndEt.text.toString())
                }
                val fangjianzhuangtaiString = fangjianzhuangtaiFbl.children.filter { it.isSelected }.map { it.toConversion<TextView>()?.text.toString() }.joinToString()
                fangjianzhuangtaiString.isNotNullOrEmpty().yes {
                    params.put("fangjianzhuangtai",fangjianzhuangtaiString)
                }
