...
import com.design.appproject.databinding.KefangyudingFilterDialogLayoutBinding
...
class ListFilterDialog(context: Context) : DrawerPopupView(context) {
    ...
    lateinit var binding: KefangyudingFilterDialogLayoutBinding
    ...
    override fun onCreate() {
        ...
        binding.apply {
            ...
            "已入住,未入住".split(",").forEachIndexed {index, s ->
                yuyuezhuangtaiFbl.addView(createCustomItemView(index,s,yuyuezhuangtaiFbl))
            }
            resetBtn.setOnClickListener { //重置
                yuyuezhuangtaiFbl.children.forEach { it.isSelected=false }
                callBackListener?.invoke(false,null)
            }
            ensureBtn.setOnClickListener { //确定
                params.clear()
                val yuyuezhuangtaiString = yuyuezhuangtaiFbl.children.filter { it.isSelected }.map { it.toConversion<TextView>()?.text.toString() }.joinToString()
                yuyuezhuangtaiString.isNotNullOrEmpty().yes {
                    params.put("yuyuezhuangtai",yuyuezhuangtaiString)
                }
