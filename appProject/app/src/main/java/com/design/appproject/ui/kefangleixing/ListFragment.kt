...
import com.design.appproject.ui.kefangleixing.ListFilterDialog
...
import com.design.appproject.databinding.KefangleixingcommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.kefangleixing.ListModel
import com.design.appproject.ui.kefangleixing.ListAdapter
 ...
 * 客房类型列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_KEFANGLEIXING)
class ListFragment : BaseBindingFragment<KefangleixingcommonListLayoutBinding>() {
    ...
    val queryIndex = Pair<String,String>("客房类型","kefangleixing")/*查询索引*/

    override fun initEvent() {
        setBarTitle("客房类型",mHasBack)
        setBarColor("#C6000F","white")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("kefangleixing","新增"))|| Utils.isAuthFront("kefangleixing","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_KEFANGLEIXING).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_KEFANGLEIXING)
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("kefangleixing",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_KEFANGLEIXING,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun KefangleixingcommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        binding.searchValueEt.text.toString().isNotNullOrEmpty().yes {
            params.put("kefangleixing","%" + binding.searchValueEt.text.toString() + "%" )
        }.otherwise {
        ...
        mIsBack.yes {
                mListModel.page("kefangleixing", requestParams)
        }.otherwise {
                mListModel.list("kefangleixing", requestParams)
        }
    }
}