...
import com.design.appproject.ui.discusskefangxinxi.ListFilterDialog
...
import com.design.appproject.databinding.DiscusskefangxinxicommonListLayoutBinding
import com.design.appproject.logic.repository.HomeRepository
import com.design.appproject.logic.viewmodel.discusskefangxinxi.ListModel
import com.design.appproject.ui.discusskefangxinxi.ListAdapter
 ...
 * 客房信息评论表列表页
 */
@Route(path = CommonArouteApi.PATH_FRAGMENT_LIST_DISCUSSKEFANGXINXI)
class ListFragment : BaseBindingFragment<DiscusskefangxinxicommonListLayoutBinding>() {
    ...
    override fun initEvent() {
        setBarTitle("客房信息评论表",mHasBack)
        setBarColor("#C6000F","white")
        binding.apply {
            addBtn.isVisible = (mIsBack && Utils.isAuthBack("discusskefangxinxi","新增"))|| Utils.isAuthFront("discusskefangxinxi","新增")
            addBtn.setOnClickListener {
                ARouter.getInstance().build(CommonArouteApi.PATH_ACTIVITY_ADDORUPDATE_DISCUSSKEFANGXINXI).navigation()
            }
            initSearch()
            ...
            mListAdapter.setOnItemChildClickListener { adapter, view, position ->
                if (view.id==R.id.edit_tv){
                  ...
                  XPopup.Builder(requireActivity()).asConfirm("提示","是否确认删除") {
                      HomeRepository.delete<Any>("discusskefangxinxi",listOf(mListAdapter.data[position].id)).observeKt {
                        it.getOrNull()?.let {
                            "删除成功".showToast()
                            loadData(1)
                        }
            ...
            srv.setAdapter(mListAdapter.apply {
                ...
                setOnItemClickListener { adapter, view, position ->
                    ArouterUtils.startFragment(CommonArouteApi.PATH_FRAGMENT_DETAILS_DISCUSSKEFANGXINXI,map=mapOf("mId" to data[position].id,"mIsBack" to mIsBack))
                }
            })
        }
    }
    private fun DiscusskefangxinxicommonListLayoutBinding.initSearch() {
    ...
    private fun loadData(page:Int,searchParams:MutableMap<String,String>?=null){
        ...
        mIsBack.yes {
                mListModel.page("discusskefangxinxi", requestParams)
        }.otherwise {
                mListModel.list("discusskefangxinxi", requestParams)
        }
    }
}